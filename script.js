...
document.addEventListener("DOMContentLoaded", () => {
    products = apiData.categories.flatMap(category => category.category_products);
...
function filterProducts(category) {
    ...
    if (category === 'All') {
        ...
        const categoryData = apiData.categories.find(cat => cat.category_name === category);
        filteredProducts = categoryData ? categoryData.category_products : [];
    }
...
function searchProducts() {
    ...
    const filteredProducts = products.filter(product => {
        const category = apiData.categories.find(cat => 
            cat.category_products.some(p => p.id === product.id)
        );
